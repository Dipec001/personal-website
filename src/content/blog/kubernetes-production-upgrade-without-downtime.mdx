...
2. **Scan for deprecated APIs in your manifests.** Use `pluto` or `kubectl deprecations` to find resources still using removed APIs:
   ...
   ```
3. **Back up etcd.** I've blogged the [routine separately](/blog/etcd-backup-routine). If you haven't taken a fresh snapshot, do it now. Upgrades that go sideways without a snapshot are how you end up restoring from yesterday.
